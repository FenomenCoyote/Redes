...
public:
    ...
    void work() {
        ...
        while(true) {
            ...
            for(int i = 4; i > 0; --i){
                std::cout << "thread [" << std::this_thread::get_id() << "] will answer in " << i << " seconds . . ." << std::endl;
                sleep(1);
            }

            buff[bytes] = '\0';
           
            std::cout << "thread [" << std::this_thread::get_id() << "] has answered " << buff;

            send(sd, buff, bytes, 0);
        }
...
int main(int argc, char* args[]){
    ...
    while(true) {
        ...
        std::thread([cliente_sd](){ ServerEmployee(cliente_sd).work(); }).detach();
    }
